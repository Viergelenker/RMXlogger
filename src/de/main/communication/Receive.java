package de.main.communication;

import java.io.IOException;
import java.io.InputStream;

import de.main.data_management.UpdateGui;
...
public class Receive {
	...
	private static void read0x04(byte[] message) {
		...
		if (CommunicationUtils.bitIsSet(message[1], 7)) {
			UpdateGui.changeEnergyStatus(true);
		} else {
			UpdateGui.changeEnergyStatus(false);
		}
	}
	...
	private static void read0x06(byte[] message) {
		...
		UpdateGui.updateBusAdress(message[1], message[2], message[3]);
	}
	...
	private static void read0x08(byte[] message) {
		...
		case 2: // Hex: 0x02
			...
			// Fahrstufen: 31
			UpdateGui.addTrain(adrLong, adrShort, opmode, (byte) 0,
	...
	private static void read0x20(byte[] message) {
		...
		UpdateGui.setTrainRmx(adrLong, adrRmx);
	}
	...
	private static void read0x24(byte[] message) {
		...
		UpdateGui.setTrainDirection(adrLong, message[4]);
		// Aktuelle Fahrstufe/Geschwindigkeit des Zuges setzen
		UpdateGui.setTrainSpeed(adrLong, message[3]);
	}
	...
	private static void read0x28(byte[] message) {
		...
		// F-Infos verarbeiten..
		UpdateGui.setTrainMode0to7(adrLong, message[3]);
		UpdateGui.setTrainMode8to15(adrLong, message[4]);
		UpdateGui.setTrainMode16to23(adrLong, message[5]);
	}
}